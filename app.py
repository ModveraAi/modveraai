...
def has_active_subscription(email: str) -> bool:
    """
    Optional safety (not required for speed). Returns True if any subscription is active.
    """
    try:
...
async def create_checkout_session(req: Request):
    ...
    # Base params (do NOT set allow_promotion_codes here)
    params = {
        "mode": "subscription",
    ...
    # Owner auto-free vs. everyone else can enter codes
...
# ====== Stripe Webhook (optional; safe no-op if not configured) ======
@app.post("/stripe/webhook")
async def stripe_webhook(request: Request):
    payload = await request.body()
    sig = request.headers.get("stripe-signature")

    # If no secret configured, accept 200 to avoid blocking
    ...
    # Handle events here if needed (checkout.session.completed, invoice.paid, etc.)
...
def unlock(email: str = None):
    """
    Manual unlock for setup/testing. Sets cookie then redirects to dashboard.
    """
...
def auth_complete(session_id: str):
    """
    Called by Stripe success_url. Sets cookie from checkout session, then redirects to dashboard.
    """
    try:
...
# ====== Dashboard (Locked/Unlocked UI + Billing) ======
...
<style>
  ...
  h2 {{margin:20px 20px 6px}}
  p.sub {{margin:0 20px 10px; color:#9aa}}
...
<body>
  <div class="top">
    <div><strong>{biz}</strong> <span class="badge">{status}</span></div>
    <div>{right_btn}</div>
  </div>
  ...
  <script>
    const unlocked = {unlocked_js};
    document.querySelectorAll('.card').forEach(card => {{
      card.addEventListener('click', () => {{
        const path = card.getAttribute('data-path');
        if (!unlocked) {{
          alert('Please subscribe to unlock these tools.');
...
""".format(
        ...
        lc=lock_class,
        billing=("" if unlocked else f"""
  <div class="billing" id="Billing">
    <h3>Billing</h3>
    <p>Choose Monthly or Annual to unlock all tools. Owner email <code>{DEV_FREE_EMAIL}</code> uses your 100% code automatically.</p>
...
# ====== Generic tool page placeholder (clicks work immediately) ======
...
<html><body style="background:#0b0b0f;color:#fff;font-family:Poppins,sans-serif">
  <div style="padding:18px;border-bottom:1px solid #222">
    <a href="/dashboard" style="color:#9aa;text-decoration:none;">&larr; Back</a>
